...
export function retrievePopularMovies(page) {
	return function (dispatch) {
		return axios.get('http://192.168.0.31:3000/getarticles')
		.then(res => {
		console.log(res)
		const result = {data: {results : res}}
			dispatch(retrievePopularMoviesSuccess(result));
		})
		.catch(error => {
...
export function retrieveNowPlayingMovies(page) {
	return function (dispatch) {
		return axios.get('http://192.168.0.31:3000/gettoparticles')
		.then(res => {
		console.log("pppppppppp")
		console.log(res)
		const result = {data: {results : res}}
			dispatch(retrieveNowPlayingMoviesSuccess(result));
		})
		.catch(error => {
